    ...
    <Transition name="panel-fade" mode="out-in">
      <div v-if="node" :key="node.id" class="node-properties">
        ...
        <template v-if="node.type === 'reference'">
          <div class="prop-item"><span class="prop-label">标题</span><a-input v-model:value="node.data.title" size="small" @change="projectStore.saveProject()" /></div>
          <div class="prop-item"><span class="prop-label">内容</span><a-textarea v-model:value="node.data.content" size="small" :rows="4" placeholder="粘贴参考资料文本..." @change="projectStore.saveProject()" /></div>
          <div class="prop-item"><span class="prop-label">来源链接</span><a-input v-model:value="node.data.sourceUrl" size="small" placeholder="https://..." @change="projectStore.saveProject()" /></div>
        </template>
      </div>
